...
- **Always** execute actions precisely as specified in the incoming JSON format: `{"azione": "ACTION_NAME", "contenuto": "CONTENT"}`.
- **Always** use the `wait` tool when necessary to ensure the UI has updated before proceeding.
- **Always** return a JSON response after completion: `{"azione": "ACTION_NAME", "risultato": "EXPLANATION", "successo": true/false}`.
