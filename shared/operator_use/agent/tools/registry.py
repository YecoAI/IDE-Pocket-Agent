...
try:
    from yecoai_security_layer import ExecutionSandboxGuard
except ImportError:
    ExecutionSandboxGuard = None
...
class ToolRegistry:
 ...
 def __init__(self):
  ...
  self._guard = ExecutionSandboxGuard() if ExecutionSandboxGuard else None
 ...
 def execute(self, name:str, params:dict)->ToolResult:
  ...
  if self._guard:
      res = self._guard.validate_tool_call(name=name, params=params)
      if not res["safe"]:
          return ToolResult.error_result(f"SECURITY BLOCKED: {res['reason']}")
