...
@Tool(name ="terminal", description ="Run a shell command and return stdout, stderr, and exit code. Use for git, package installs, running scripts, checking processes, or any CLI task. Commands run from the codebase root. Destructive commands are blocked. For long outputs, results are truncated.", model =Terminal)
