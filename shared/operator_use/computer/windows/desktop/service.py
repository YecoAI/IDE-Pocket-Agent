...
class Desktop:
 ...
 def get_state(self, as_bytes:bool =False, format:str ="PNG", quality:int =80, resize_width:int =None)->DesktopState:
 ...
 if nodes:
 screenshot =self.get_annotated_screenshot(nodes =nodes, as_bytes =as_bytes, format =format, quality =quality, resize_width =resize_width)
 else:
 screenshot =self.get_screenshot(as_bytes =as_bytes, format =format, quality =quality, resize_width =resize_width)
 else:
 screenshot =self.get_screenshot(as_bytes =as_bytes, format =format, quality =quality, resize_width =resize_width)
 else:
 screenshot =None
 ...
 def get_screenshot(self, as_bytes:bool =False, format:str ="PNG", quality:int =80, resize_width:int =None)->bytes |Image.Image:
 try:
 ...
 ratio =resize_width /screenshot.width
 ...
 save_img =screenshot.convert("RGB")if format.upper()in["JPEG", "JPG"]else screenshot
 save_img.save(buffered, format =format, quality =quality)
 screenshot_bytes =buffered.getvalue()
 buffered.close()
 return screenshot_bytes
 return screenshot

 def get_annotated_screenshot(self, nodes:list[TreeElementNode], as_bytes:bool =False, format:str ="PNG", quality:int =80, resize_width:int =None)->bytes |Image.Image:
 ...
 save_img =padded_screenshot.convert("RGB")if format.upper()in["JPEG", "JPG"]else padded_screenshot
 save_img.save(buffered, format =format, quality =quality)
 screenshot_bytes =buffered.getvalue()
 buffered.close()
 return screenshot_bytes
